class CountdownTimer extends Phaser.GameObjects.Container {
    ...
    constructor (scene, x, y, initialTime, children) {
        ...
        const smallSpace = 10;
        const timerText = this.scene.add.text(0, 0, "Timer: ", { fill: GameConstants.TEXT.color });
        this.timeDisplay = this.scene.add.text(timerText.width + smallSpace, 0, initialTime.toString(), { fill: GameConstants.TEXT.color });
        this.add([timerText, this.timeDisplay]);

        // Set a timer to tick every second
        this.timedEvent = this.scene.time.addEvent({ delay: 1000, callback: this.onTimerEvent, callbackScope: this, loop: true });
